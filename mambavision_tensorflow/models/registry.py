...
# Global dictionaries for model registration
...
def register_pip_model(fn):
    """Decorator to register a model and handle pip-specific logic."""
...
def register_model(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        model_name = kwargs.get('model_name', fn.__name__)
        _model_entrypoints[model_name] = fn
        _model_to_module[model_name] = fn.__module__
        _module_to_models[fn.__module__].add(model_name)
        return model
    return wrapper
...
def is_model_default_key(model_name, cfg_key):
    return model_name in _model_default_cfgs and _model_default_cfgs[model_name].get(cfg_key, False)
...
def load_checkpoint(model, checkpoint_path, use_ema=False, strict=True):
    """Load weights into the model from checkpoint."""
